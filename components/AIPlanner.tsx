...
import { LessonPlanSkeleton } from './LoadingSkeleton.tsx';
               ...
               <div className="flex justify-between items-center mb-10 pb-6 border-b border-slate-100 print:hidden">
                 ...
                 <div className="flex space-x-3">
                    ...
                    <button onClick={handleExportWord} className="bg-blue-50 text-blue-700 px-6 py-3 rounded-xl font-bold flex items-center space-x-2 hover:bg-blue-100 transition-all">
                     ...
                     <button onClick={() => { if(plan) { const t = `📋 Lesson Plan: ${sport} - ${topic}\nGrade: ${grade} | Duration: ${duration}\n\n✅ Objectives:\n• ${plan.objectives?.know || ""}\n\n🏃 Activity: ${plan.mainActivity?.activities?.[0]?.title || ""}\n\nGenerated by SmartPE India`; window.open(`https://wa.me/?text=${encodeURIComponent(t)}`, "_blank"); }}} className="bg-emerald-600 text-white px-6 py-3 rounded-xl font-bold flex items-center space-x-2 shadow-lg hover:bg-emerald-700 transition-all"><span>📱 WhatsApp</span></button>
